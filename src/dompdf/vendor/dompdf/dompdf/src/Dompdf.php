...
class Dompdf
{
     ...
     * @var array
     */
    private $allowedLocalFileExtensions = [
        "htm",
        "html",
    ];
    ...
    public function loadHtmlFile($file, $encoding=null)
    {
        ...
        ] = Helpers::getFileContent($uri, $this->httpContext);
        if (empty($contents)) {
            throw new Exception("File '$file' not found.");
        }
    ...
    public function loadHtml($str, $encoding=null)
    {
        ...
        } else if (isset($document_encoding) === false && strpos($str, '<head>') !== false) {
            $str = str_replace('<head>', '<head><meta http-equiv="Content-Type" content="text/html;charset=UTF-8">', $str);
        } else if (isset($document_encoding) === false) {
    ...
    public function render()
    {
        ...
        if ($defaultOptionPaperSize[2] !== $paperSize[2]
    ...
    private function write_log()
    {
        ...
        $out = sprintf(
            "<span style='color: #000' title='Frames'>%6d</span>"."<span style='color: #009' title='Memory'>%10.2f KB</span>"."<span style='color: #900' title='Time'>%10.2f ms</span>"."<span  title='Quirksmode'>  ".($this->quirksmode ? "<span style='color: #d00'> ON</span>" : "<span style='color: #0d0'>OFF</span>")."</span><br />",
            $frames,
            $memory,
            $time
        );
