...
class Stylesheet
{
    ...
    private function _specificity($selector, $origin=self::ORIG_AUTHOR)
    {
        ...
        $c = min(
            (mb_substr_count($selector, ".") + mb_substr_count($selector, "[")),
            255
        );

        $d = min(
            (mb_substr_count($selector, " ") + mb_substr_count($selector, ">") + mb_substr_count($selector, "+") + mb_substr_count($selector, "~") - mb_substr_count($selector, "~=")),
            255
        );
    ...
    private function _css_selector_to_xpath(string $selector, bool $first_pass=false): array
    {
        ...
        while ($i < $len) {
            ...
            case ":":
                ...
                default:
                    $query = "/../..";
                    // go up two levels because generated content starts on the body element
                    $tok = "";
                    break;
                }//end switch
                break;
                ...
                case "media":
                    ...
                    foreach ($media_queries as $media_query) {
                        if (in_array($media_query, $acceptedmedia)) {
                            ...
                            break;
                        } else if (!in_array($media_query, self::$VALID_MEDIA_TYPES)) {
