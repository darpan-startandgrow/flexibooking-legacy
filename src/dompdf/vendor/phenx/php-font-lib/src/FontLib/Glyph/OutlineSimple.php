...
class OutlineSimple extends Outline
{
    ...
    public function encodePoints($points)
    {
        ...
        // yMax
        // Simple glyf
        $l += $font->w([self::uint16, count($endPtsOfContours)], $endPtsOfContours);
        // endPtsOfContours
        $l += $font->writeUInt16(0);
        // instructionLength
        $l += $font->w([self::uint8, count($flags)], $flags);
        // flags
        $l += $font->w([self::int16, count($coords_x)], $coords_x);
        // xCoordinates
        $l += $font->w([self::int16, count($coords_y)], $coords_y);
        // yCoordinates
        return $l;
